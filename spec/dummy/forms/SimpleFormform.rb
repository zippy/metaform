    ...
    Form.def_listings do
      ...
      Form.listing 'search_filters', 
        ...
        :search_rules => {
          'doula_name_is' => Proc.new{|search_for| ":name = '#{search_for}'"},
          'doula_age_is' => Proc.new{|search_for| ":age = '#{search_for}'"},
          'doula_name_begins_with' => {:regex => true, :block => Proc.new{|search_for| ":name #{Metaform.ilike} '#{search_for}%' "}},
          'doula_name_ends_with' => {:regex => true, :block => Proc.new{|search_for| ":name #{Metaform.ilike} '%#{search_for}' "}},
          'doula_name_begins_with_j_or_b' => {:regex => true, :block => Proc.new{|search_for| ":name #{Metaform.ilike} 'J%' or :name #{Metaform.ilike} 'B%' "}}}
