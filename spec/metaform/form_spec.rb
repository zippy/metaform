...
describe SimpleForm do
  ...
  describe "-- dsl: " do
    ...
    describe "listing" do
      before(:each) do
        rec = Record.make(SimpleForm.new,'create',{:name =>'Bob Smith',:age => '23',:married => 'n'})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Joe Smith',:age => '30',:married => 'y'})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Will Smith',:age => '35',:married => 'y'})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Jane Smith',:age => '55',:married => 'y'})
        rec.save('simple')      
      end

      it "should return 4 items with no limits on the search" do 
        the_list = @form.listings['plain']
        the_list.get_records.size.should == 4
      end
      
      it "should return the number of items that match a workflow_state filter" do 
         the_list = @form.listings['plain']
         the_list.workflow_state_filter = 'logged'
         the_list.get_records.size.should == 3
      end
      
      it "should return the number of items that match a ruby filter" do
        puts "Finding:  #{Record.locate(:all,:filters=>[":name =~ /^J/"]).size}"
        the_list = @form.listings['filter_by_name']
        @search_params = {}
        the_list.get_records.size.should == 2
      end
...
#       end
#       
#       it "should evalute boolean conditions correctly" do
#         @form.with_record(@record) do
#           @record.age = '3'
#           @form.conditions['age<15 or age>75'].evaluate.should == true
#           @form.conditions['age>15 and age<75'].evaluate.should == false
#           @form.conditions['age>15 and age<75 or age=99'].evaluate.should == false
#           @record.age = '20'
#           @form.conditions['age<15 or age>75'].evaluate.should == false
#           @form.conditions['age>15 and age<75'].evaluate.should == true
#           @form.conditions['age>15 and age<75 or age=99'].evaluate.should == true
#           @record.age = '80'
#           @form.conditions['age<15 or age>75'].evaluate.should == true
#           @form.conditions['age>15 and age<75'].evaluate.should == false
#           @form.conditions['age>15 and age<75 or age=99'].evaluate.should == false
...
#       end
#       it "should raise an error if defining a duplicate set or enum constraint label" do
