...
describe SimpleForm do
  ...
  describe "-- dsl: " do
    ...
    describe "listing with locate" do
      before(:each) do
        rec = Record.make(SimpleForm.new,'create',{:name =>'Bob Smith',:age => '30',:married => 'n', :favorite_date => Time.now - 1.year})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Joe Smith',:age => '30',:married => 'y', :favorite_date => Time.now - 1.day})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Will Smith',:age => '35',:married => 'y', :favorite_date => Time.now - 1.month})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Jane Doe',:age => '55',:married => 'y', :favorite_date => Time.now - 1.minute})
        rec.save('simple')      
        @params = {}
      end
      ...
      it "should return 4 items when all is chosen" do 
        the_list = Form.listings['locate_filters']
      ...
      it "should return the number of items that match a ruby filter" do
        ...
        @params[:search] = {"on_main"=>"doula_name_begins_with_j_or_b", "for_main"=>"dummy"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 3
      end
      
      it "should return the number of items that match a combination of ruby filters" do
        the_list = Form.listings['locate_filters']
        @params[:search] = {"on_main"=>"doula_name_begins_with", "for_main"=>"J", "on_end"=>'doula_name_ends_with', "for_end" => 'e Smith'}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 1
        @params[:search] = {"on_main"=>"doula_name_is", "for_main"=>"Bob Smith", "on_age" => 'doula_age_is', "for_age" => '30'}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 1
      end
      
      
      it "should return the number of items that match a filter generated by the listing from a given pair" do
        the_list = Form.listings['locate_generate_filters_by_name']
        @params[:search] = {"on_main"=>"doula_name_b", "for_main"=>"J"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 2
        @params[:search] = {"on_main"=>"doula_name_c", "for_main"=>"Smith"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 3
        @params[:search] = {"on_main"=>"doula_name_is", "for_main"=>"Bob Smith"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 1
        @params[:search] = {"on_main"=>"doula_name_not", "for_main"=>"Bob Smith"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 3
      end
      
      it "should return the number of items that match a filter, taken from a list of FormInstances if pre-set" do
        the_list = Form.listings['locate_filters']
        the_list.records = FormInstance.find(:all, :conditions => {:id => 0..2})
        @params[:search] = {"on_main"=>"doula_name_begins_with", "for_main"=>"J"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 1
      end
      
      it "should only prefill an AnswersHash with fields specified in the fields parameter" do
        the_list = Form.listings['locate_filters']
        the_list.records = FormInstance.find(:all, :conditions => ['(form_instances.id = 1 or form_instances.id = 2) and field_instances.field_id in (?)',['name']], :include => [:field_instances])
        the_list.return_answers_hash = true
        @params[:search] = {"on_main"=>"doula_name_begins_with", "for_main"=>"J"}
        lambda {(@records,@search_params) = the_list.fill_records(@params);@records[0].married}.should raise_error
        lambda {(@records,@search_params) = the_list.fill_records(@params);@records[0].name}.should_not raise_error
      end
    ...
    describe "listing with search" do
      before(:each) do
        rec = Record.make(SimpleForm.new,'create',{:name =>'Bob Smith',:age => '30',:married => 'n', :favorite_date => Time.now - 1.year})
        rec.save('simple',{:workflow_action=>'create'})        
        rec = Record.make(SimpleForm.new,'create',{:name =>'Joe Smith',:age => '30',:married => 'y', :favorite_date => Time.now - 1.day})
      ...
      it "should return 4 items when all is chosen" do 
        the_list = Form.listings['search_filters']
        @params[:search] = {'on_main' => 'all', 'for_main' => '' }
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 4
      end
      ...
      it "should return the number of items that match a fixed workflow_state filter" do 
         the_list = Form.listings['search_plain_with_workflow_state_filter']
         @params[:search] = {'on_main' => 'workflow_state_is_logged_or_dummy', 'for_main' => 'dummy'}
         (@records,@search_params) = the_list.fill_records(@params)
         @records.size.should == 3
      end
      ...
      it "should return the number of items that match a filter generated by the listing from a given pair" do
        the_list = Form.listings['search_generate_filters_by_name']
        @params[:search] = {"on_main"=>"doula_name_b", "for_main"=>"J"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 2
        @params[:search] = {"on_main"=>"doula_name_c", "for_main"=>"Smith"}
      ...
      it "should return the number of items that match a filter and a manual search filter from the params" do
        the_list = Form.listings['search_with_manual_filter']
        @params[:search] = {"on_main"=>"doula_name_begins_with", "for_main"=>"J"}
        (@records,@search_params) = the_list.fill_records(@params)
        @records.size.should == 2
...
#      end
    ...
    describe "q (display a question)" do
      ...
      it "should render the q with the value determined by the index returned by the flow_through proc" do
        ...
        @form.get_body.should == ["<div id=\"question_house_value\" class=\"question\"><label class=\"label\" for=\"record_house_value\">House value:</label><input id=\"record_house_value\" name=\"record[house_value]\" type=\"text\" value=\"100\" /></div>"]
      end
      ...
      it "should use the current index to determine the value for the follow up if the flow_through proc returns nil" do
        ...
        @form.with_record(@record,:render) do
          @record.save('create')
          # if the current index is not set, it defaults to 0.
          @form.set_current_index(3)
    ...
    describe "build (render form)" do
      ...
      it "should generate html for a simple presentation forced read only" do
        @form.build('name_only',@record,0,true).should == [
          "<div id=\"presentation_name_only\" class=\"presentation\">\n<div id=\"question_name\" class=\"question\"><label class=\"label\" for=\"record_name\">Name:</label><span id=\"record_name\">Bob Smith</span></div>\n</div>\n<input type=\"hidden\" name=\"meta[force_read_only]\" id=\"meta_force_read_only\" value=\"1\">\n<input type=\"hidden\" name=\"meta[last_updated]\" id=\"meta_last_updated\" value=0>",
          ""
        ]
      end
